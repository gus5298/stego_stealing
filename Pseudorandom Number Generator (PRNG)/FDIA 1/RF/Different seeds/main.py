...
def process_folder(input_folder, model_path, output_excel):
    model = joblib.load(model_path)  # Load Random Forest model
    results = []
    ...
    for idx, filename in enumerate(image_files):
        ...
        # === Random Forest Prediction BEFORE decoding === #
        flat_input = final_stego.flatten().reshape(1, -1)
        pred_class_idx = int(model.predict(flat_input)[0])
        pred_probs = model.predict_proba(flat_input)[0]
        pred_class = CLASS_NAMES[pred_class_idx]
        confidence = float(pred_probs[pred_class_idx])

        # === Now Decode Message AFTER classification === #
    ...
    cm = confusion_matrix(df["True Label"], df["Predicted Class"], labels=CLASS_NAMES)
