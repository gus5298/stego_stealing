...
# === VISUALIZATION === #
def save_visualization_overlay(image, coords, save_path):
    vis_image = cv2.cvtColor(image.copy(), cv2.COLOR_GRAY2BGR)
    for y, x in coords:
        if 0 <= y < vis_image.shape[0] and 0 <= x < vis_image.shape[1]:
            vis_image[y, x] = (0, 0, 255)  # Red
    cv2.imwrite(save_path, vis_image)
    print(f"✅ Saved red overlay: {os.path.basename(save_path)}")

# === MAIN FUNCTION === #
def process_folder(input_folder, model_path, stego_output_folder, results_output_folder, seed_secret_key, message_length_chars):
    global NUM_PIXELS, MESSAGE_BITS
    MESSAGE_BITS = message_length_chars * 8
    ...
    for idx, filename in enumerate(image_files):
        ...
        # --- CNN Prediction on Original Image --- #
        orig_pred_class, orig_conf = analyze_with_cnn(image, model)
        original_predictions.append({
            "Image": filename,
            "True Label": "Normal",
            "CNN Predicted Class": orig_pred_class,
        ...
        original_msg = generate_random_message(chars=message_length_chars)
        msg_bits = string_to_bits(original_msg)

        if len(msg_bits) > (image.size - SEED_RESERVED_PIXELS):
            raise ValueError(f"Message too long to embed. Available bits: {image.size - SEED_RESERVED_PIXELS}, required: {len(msg_bits)}")

        prng_pixels = generate_prng_pixel_positions(image.shape, len(msg_bits), seed_value)
        ...
        visual_path = os.path.join(visualization_folder, f"visual_{filename}")
        save_visualization_overlay(final_stego, prng_pixels, visual_path)
        ...
        results.append({
            ...
            "Message Length": message_length_chars,
            "Seed": seed_value,
    ...
    with pd.ExcelWriter(output_excel, engine='openpyxl') as writer:
        ...
        pd.DataFrame(original_predictions).to_excel(writer, index=False, sheet_name="Original Image Results")
    ...
    original_cm = confusion_matrix(original_df["True Label"], original_df["CNN Predicted Class"], labels=CLASS_NAMES)

    plt.figure(figsize=(6, 5))
    sns.heatmap(original_cm, annot=True, fmt='d', cmap="Greens", xticklabels=CLASS_NAMES, yticklabels=CLASS_NAMES)
    plt.xlabel("Predicted Label")
    plt.ylabel("True Label")
    ...
    print(f"\n✅ All results and visualizations saved to: {results_output_folder}")

# === RUN === #
message_length_chars = int(input("Enter the number of characters to encode: "))

input_folder = "PRNG/MMS/MMS Images/Faulty"
model_path = "PRNG/MMS/cnn_best_model.h5"
stego_output_folder = "PRNG/MMS/Custom Message Encoder/Faulty/stego_images (3 letters)"
results_output_folder = "PRNG/MMS/Custom Message Encoder/Faulty/Final Result (3 letters)"
seed_secret_key = "Gustavo_Sanchez"

process_folder(input_folder, model_path, stego_output_folder, results_output_folder, seed_secret_key, message_length_chars)
