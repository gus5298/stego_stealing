...
def shap_pixel_selection(image, explainer, num_pixels):
...
def save_pixel_overlay(image, coords, output_path):
    overlay = cv2.cvtColor(image.copy(), cv2.COLOR_GRAY2BGR)
    for y, x in coords:
        cv2.circle(overlay, (x, y), radius=1, color=(0, 0, 255), thickness=-1)
    cv2.imwrite(output_path, overlay)

def save_metadata_overlay(image, reserved_bits, output_path):
    overlay = cv2.cvtColor(image.copy(), cv2.COLOR_GRAY2BGR)
    flat = image.flatten()
    for i in range(reserved_bits):
        y = i // image.shape[1]
        x = i % image.shape[1]
...
# === MAIN PROCESS === #
def process_images(folder_path, model_path, output_excel, stego_output_folder, message_length):
    shap_overlay_folder = os.path.join(stego_output_folder, "shap_overlays")
    os.makedirs(shap_overlay_folder, exist_ok=True)
    ...
    if 'normal' in folder_path_lower:
        true_label = 'normal'
    elif 'attack' in folder_path_lower:
        true_label = 'attack'
    else:
        raise ValueError(f"❌ Could not determine true label from folder path: {folder_path}")
    ...
    for i, image in enumerate(images):
        ...
        original_pred = str(pred_orig).lower()

        selected_pixels = shap_pixel_selection(image, explainer, num_pixels=total_bits)
        ...
        if len(coord_bits) > RESERVED_BITS:
            raise ValueError("Metadata size exceeds reserved bit capacity.")
        ...
        overlay_path = os.path.join(shap_overlay_folder, f"overlay_{filenames[i]}")
        save_pixel_overlay(image, selected_pixels, overlay_path)

        metadata_overlay_path = os.path.join(stego_output_folder, f"metadata_overlay_{filenames[i]}")
        if message_length > 0:
            save_metadata_overlay(image, RESERVED_BITS, metadata_overlay_path)
        ...
        print(f"  Match Status    : {'✅ Matched' if message == decoded_message else '❌ Mismatched'}")
        ...
        stego_pred = str(pred_stego).lower()
    ...
    cm = confusion_matrix(df["True Label"], df["RF Predicted Class"], labels=class_labels_lower)
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=[label_map[l] for l in class_labels_lower])
    fig, ax = plt.subplots(figsize=(6, 5))
    disp.plot(ax=ax, cmap="Blues", values_format='d')
    ax.set_title("Confusion Matrix (After Embedding)")
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "confusion_matrix.png"))
    plt.close()

    cm_orig = confusion_matrix(df["True Label"], df["RF Predicted Class (Original)"], labels=class_labels_lower)
    disp_orig = ConfusionMatrixDisplay(confusion_matrix=cm_orig, display_labels=[label_map[l] for l in class_labels_lower])
    fig, ax = plt.subplots(figsize=(6, 5))
    disp_orig.plot(ax=ax, cmap="Greens", values_format='d')
    ax.set_title("Confusion Matrix (Original Images)")
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    output_excel = "Layered IG LSB/RF FDIA 1/Custom Message Encoder/Normal/ig_layered_stego_results(4 letters).xlsx"
    stego_output_folder = "Layered IG LSB/RF FDIA 1/Custom Message Encoder/Normal/stego_images(4 letters)"

    if not os.path.exists(stego_output_folder):
        os.makedirs(stego_output_folder)

    message_length = int(input("Enter number of characters to encode in each image: "))
    process_images(input_folder, model_path, output_excel, stego_output_folder, message_length)
